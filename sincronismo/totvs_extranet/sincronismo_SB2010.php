<?php
	...
	if($rowTotvsLog["TP_LOG"] == "Insert"){
	    ...
	    mysql_query("UPDATE tb_pcp_produto_saldo SET
					 ...
					 WHERE CO_RECNO = '".$rowTotvsSB2010['R_E_C_N_O_']."'",$conexaoExtranet);
			
	}elseif($rowTotvsLog["TP_LOG"] == "Update Campo D_E_L_E_T_"){
		
	    if(trim($rowTotvsSB2010['D_E_L_E_T_'])=='*'){
		
	    	mysql_query("UPDATE tb_pcp_produto_saldo SET FL_DELET = '*' WHERE CO_RECNO = '".$rowTotvsSB2010['R_E_C_N_O_']."'",$conexaoExtranet);
			
	    }else{
		
	    	mysql_query("UPDATE tb_pcp_produto_saldo SET FL_DELET = null WHERE CO_RECNO = '".$rowTotvsSB2010['R_E_C_N_O_']."'",$conexaoExtranet);

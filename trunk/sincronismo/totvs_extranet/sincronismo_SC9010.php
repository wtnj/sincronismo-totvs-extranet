	...
	if($rowTotvsLog["TP_LOG"] == "Insert"){
	    ...
	    mysql_query("INSERT INTO tb_fat_pedido_venda_liberado (NU_PEDIDO_VENDA 	
			 	     ...
			 	     VALUES('".$rowTotvsSC9010['C9_PEDIDO']."'  
						 ...
						 , '".$rowTotvsSC9010['R_E_C_N_O_']."')",$conexaoExtranet);
	    ...
	    if(trim($rowTotvsSC9010['D_E_L_E_T_'])=='*'){
	    	...
	    	mysql_query("UPDATE tb_fat_pedido_venda_liberado SET FL_DELET = '*' WHERE CO_RECNO = '".$rowTotvsSC9010['R_E_C_N_O_']."'",$conexaoExtranet);
			
	    }else{
			
	    	mysql_query("UPDATE tb_fat_pedido_venda_liberado SET FL_DELET = null WHERE CO_RECNO = '".$rowTotvsSC9010['R_E_C_N_O_']."'",$conexaoExtranet);

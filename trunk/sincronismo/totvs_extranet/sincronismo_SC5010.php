<?php
	...
	if($rowTotvsLog["TP_LOG"] == "Insert"){
	    ...
	    if(trim($rowTotvsSC5010['D_E_L_E_T_']) == '*'){
	    	...
	    	mysql_query("UPDATE tb_fat_pedido_venda SET FL_DELET = '*' WHERE CO_RECNO = '".$rowTotvsSC5010['R_E_C_N_O_']."'",$conexaoExtranet)or die(mysql_error());
			
	    }else{
			
	    	mysql_query("UPDATE tb_fat_pedido_venda SET FL_DELET = null WHERE CO_RECNO = '".$rowTotvsSC5010['R_E_C_N_O_']."'",$conexaoExtranet)or die(mysql_error());
